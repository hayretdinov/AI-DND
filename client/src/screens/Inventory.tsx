...
export function Inventory({ onClose, onOpenMap, onOpenJournal, onOpenSettings }: InventoryProps) {
  ...
  const maxWeight = player ? getPlayerCarryCapacity(player, inventory) : inventory.maxCarryWeight;
  const weightRatio = Math.min(100, (currentWeight / maxWeight) * 100);
  const isOverloaded = currentWeight >= maxWeight;
  const attributes = player ? resolveEffectivePlayerStats(player, inventory) : undefined;
